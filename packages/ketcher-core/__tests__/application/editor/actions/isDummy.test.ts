...
describe('attribute operations isDummy()', () => {
  ...
  // Fork behaviour, operation-null-guards: upstream expects false here, which
  // makes the action keep an operation whose execute() then dereferences the
  // missing atom. AtomAttr.execute guards that lookup in this fork and isDummy
  // reports true, so the action drops the operation and the undo stack stays
  // clean. See __tests__/unc-regression/operation-null-guards.test.ts.
  it('AtomAttr does not throw and is dummy when the atom is gone', () => {
